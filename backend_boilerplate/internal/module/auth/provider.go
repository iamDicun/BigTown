...
func NewProvider(db *sql.DB, jwtSecret string) *Provider {
	return &Provider{db: db, jwtSecret: jwtSecret}
}
...
func (p *Provider) Usecase() *usecase.AuthUsecase {
	if p.usecase == nil {
		p.usecase = usecase.NewAuthUsecase(p.db, p.AuthRepository(), p.UserReader(), p.jwtSecret)
	}
	return p.usecase
}
