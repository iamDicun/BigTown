...
func (r *AuthRepository) CreateUserIdentityWithTx(ctx context.Context, tx *sql.Tx, identity entity.UserIdentity) error {
	_, err := tx.ExecContext(
		ctx,
		`INSERT INTO user_identities (user_id, provider, external_subject, tenant_id, email)
		 VALUES ($1, $2, $3, $4, $5)
		 ON CONFLICT (provider, tenant_id, external_subject) DO NOTHING`,
		identity.UserID,
		identity.Provider,
...
func (r *AuthRepository) FindUserIdentity(ctx context.Context, provider string, tenantID string, externalSubject string) (*entity.UserIdentity, error) {
	var identity entity.UserIdentity
	err := r.db.QueryRowContext(
		ctx,
		`SELECT id::text, user_id::text, provider, external_subject, tenant_id, email
		 FROM user_identities
		 WHERE provider = $1 AND tenant_id = $2 AND external_subject = $3`,
		provider,
		tenantID,
		externalSubject,
	).Scan(&identity.ID, &identity.UserID, &identity.Provider, &identity.ExternalSubject, &identity.TenantID, &identity.Email)
	if err != nil {
		return nil, err
	}
	return &identity, nil
