...
Lưu hồ sơ user nội bộ của BigTown. Đây là user được dùng bởi game, API và phân quyền nội bộ, bất kể user đăng nhập bằng email/password hay Microsoft Teams SSO.
...
- `app_user` chỉ giữ hồ sơ user nội bộ.
- Không lưu password hash trong bảng này.
- Thông tin gameplay nên nằm ở `characters`.

### 3.2 `credential`

Lưu credential cho luồng đăng nhập local bằng email/password.
...
- User đăng nhập bằng Teams SSO có thể không có dòng trong `credential`.
- Nếu một Teams user sau này cần login local, có thể thêm credential cho cùng `app_user`.

### 3.3 `user_identities`

Lưu mapping giữa user nội bộ và identity từ hệ thống ngoài, hiện tại là Microsoft Teams/Microsoft Entra ID.

```text
id                  uuid primary key
user_id             uuid not null references app_user(id)
provider            varchar not null      -- teams
external_subject    varchar not null      -- oid từ Microsoft token
tenant_id           varchar not null      -- tid từ Microsoft token
email               varchar
created_at          timestamptz not null
updated_at          timestamptz not null
```
...
- Teams SSO token chỉ dùng để chứng minh người dùng là ai.
- Backend verify Teams token bằng Microsoft Entra JWKS, sau đó tìm hoặc tạo `app_user` và `user_identities`.
- Sau khi xác thực Teams xong, backend vẫn cấp BigTown JWT nội bộ để dùng cho REST API và Centrifuge.
- Không dùng trực tiếp Teams token cho mọi request gameplay.
...
### 8.1 Xác thực bằng Teams SSO

```text
User mở app trong Microsoft Teams
Frontend dùng Teams SDK lấy SSO token
Frontend gửi POST /api/auth/teams {sso_token}
Backend verify token qua Microsoft Entra JWKS
Backend đọc user_identities theo provider=teams, tenant_id=tid, external_subject=oid
Nếu identity đã tồn tại:
  - Load app_user tương ứng
Nếu identity chưa tồn tại:
  - Tìm app_user theo email
  - Nếu chưa có app_user thì tạo app_user mới
  - Tạo user_identities để link Teams identity với app_user
Backend cấp BigTown access_token và refresh_token nội bộ
Frontend dùng BigTown access_token cho REST API và Centrifuge
```

Ghi chú:

- Teams SSO token không phải token gameplay lâu dài.
- BigTown JWT nội bộ vẫn là token chính sau khi backend xác thực xong Teams token.
- Local email/password auth vẫn giữ để phục vụ dev ngoài Teams.

### 8.2 Vào game

```text
Client gọi API lấy profile
Backend đọc app_user, characters, inventory, equipment
Client mở Centrifuge WebSocket bằng BigTown access_token
...
### Centrifuge realtime events

Centrifuge connection endpoint:

```text
WS /connection/websocket
```

Client gửi BigTown `access_token` làm Centrifuge connection token. Client subscribe/publish vào room channel, ví dụ:
